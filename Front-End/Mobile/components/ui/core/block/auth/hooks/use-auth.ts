...
import { setLogin, setLogout } from '@/components/ui/core/block/auth/store/auth-store';
import type { AuthResponse } from '@/components/ui/core/block/auth/types/auth-types';
      ...
      onSubmit: async ({ value, formApi }) => {
        try {
          ...
          await handleApiError(error, formApi, (message) => {
            toast.error({ title: 'Gagal masuk', message: message });
          });
        }
      },
    });
