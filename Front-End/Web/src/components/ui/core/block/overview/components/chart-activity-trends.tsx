...
export function ChartActivityTrends({
  ...
  const totals = React.useMemo(
    () => ({
      antrian: filteredData.reduce((acc, curr) => acc + curr.antrian, 0),
      pasien: filteredData.reduce((acc, curr) => acc + curr.pasien, 0),
    }),
    [filteredData]
  )
  ...
  return (
    <Card className={cn("grid pt-0", className)}>
      <CardHeader className="flex items-center justify-between gap-2 px-6 space-y-0 border-b py-4 sm:flex-row">
        <div className="grid flex-1 gap-1">
          <CardTitle className="line-clamp-1">{title}</CardTitle>
          <CardDescription className="line-clamp-1">
            {description}
        ...
        <CardAction className="flex items-center gap-7">
          {/* Summary Stats */}
          <div className="items-center gap-4 text-sm md:flex">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-medium">{totals.antrian}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div
                className="h-2.5 w-2.5 rounded-full"
              ...
              <span className="font-medium">{totals.pasien}</span>
            </div>
          </div>
